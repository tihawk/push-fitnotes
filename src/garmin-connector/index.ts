...
export class GarminConnector {
    ...
    constructor(config: GarminConnectorConfigI, logger) {
        ...
        this.config.username ? credentials = {username: this.config.username, password: this.config.password} : undefined
        this.GCClient = new GarminConnect()
    }

    async logIntoGarminConnect() {
        await this.GCClient.login();
    }
